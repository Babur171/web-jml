...
export default {
  ...
  methods: {
    ...
    async getData(searchText = "") {
      const url =
        config.apiUrl +
        `transactions/all?type=earning&type=borrow&type=availGiftCard&&page=${
          this.paginationOptions.current
        }&limit=${this.paginationOptions.perPage}${
          searchText ? `&search=${searchText}` : ""
        }`;
      const response = await apiClients.get(url);
      this.siteEarning = response.data.transactions;
      this.paginationOptions.total = response.data.count;
    },
    async getStatistics() {
      const url =
        config.apiUrl +
        `transactions/kpis?type=earning&type=borrow&type=availGiftCard`;
      const response = await apiClients.get(url);
