...
import Payment from '@/views/table/advanced/Payment';
...
export default {
  ...
  methods: {
    ...
    async getData(searchText = '') {
      const url =
        config.apiUrl +
        `transactions/all?page=${this.paginationOptions.current}&type=vendorPayment&limit=${
          this.paginationOptions.perPage
        }${searchText ? `&search=${searchText}` : ''}`;
      const response = await apiClients.get(url);
      this.vendorPayments = response.data.transactions;
      this.paginationOptions.total = response.data.count;
    },
    async getStatistics() {
      const url = config.apiUrl + `transactions/kpis?type=vendorPayment`;
      const response = await apiClients.get(url);
    ...
    setStatistics() {
      this.statistics = [
        ...
        {
          name: shapeLine2,
          title: 'This Month Vendor Payment',
          count: this.statisticsData.currentMonthPointsandCash,
          bg: 'bg-[#E5F9FF] dark:bg-slate-900	',
        },
        {
          name: shapeLine2,
          title: 'Last Vendor Payment',
